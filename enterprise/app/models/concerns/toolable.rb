module Concerns::Toolable
  ...
  def add_base_headers(headers, state)
    headers['X-StravoX-Account-Id'] = state[:account_id].to_s if state[:account_id]
    headers['X-StravoX-Assistant-Id'] = state[:assistant_id].to_s if state[:assistant_id]
    headers['X-StravoX-Tool-Slug'] = slug if slug.present?
  end

  def add_conversation_headers(headers, conversation)
    headers['X-StravoX-Conversation-Id'] = conversation[:id].to_s if conversation[:id]
    headers['X-StravoX-Conversation-Display-Id'] = conversation[:display_id].to_s if conversation[:display_id]
  end

  def add_contact_headers(headers, contact)
    headers['X-StravoX-Contact-Id'] = contact[:id].to_s if contact[:id]
    headers['X-StravoX-Contact-Email'] = contact[:email].to_s if contact[:email].present?
    headers['X-StravoX-Contact-Phone'] = contact[:phone_number].to_s if contact[:phone_number].present?
  end
