...
RSpec.describe Account, type: :model do
    ...
    describe 'when captain limits are not configured' do
      it 'returns default values' do
        ...
        expect(account.captain_monthly_limit).to eq(
          { documents: StravoXApp.max_limit, responses: StravoXApp.max_limit }.with_indifferent_access
        )
      end
    end
  ...
  describe 'subscribed_features' do
    ...
    before do
      InstallationConfig.where(name: 'STRAVOX_CLOUD_PLAN_FEATURES').first_or_create(value: plan_features)
    end
