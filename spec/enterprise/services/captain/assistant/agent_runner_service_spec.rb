  ...
  describe '#generate_response' do
    ...
    context 'when an error occurs' do
      ...
      before do
        ...
        allow(StravoXExceptionTracker).to receive(:new).and_return(
          instance_double(StravoXExceptionTracker, capture_exception: true)
        )
      end

      it 'captures exception and returns error response' do
        expect(StravoXExceptionTracker).to receive(:new).with(error, account: conversation.account)
      ...
      context 'when conversation is nil' do
        ...
        it 'handles missing conversation gracefully' do
          expect(StravoXExceptionTracker).to receive(:new).with(error, account: nil)
