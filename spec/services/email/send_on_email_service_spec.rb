...
describe Email::SendOnEmailService do
  ...
  describe '#perform' do
    ...
    context 'when an error occurs' do
      ...
      let(:exception_tracker) { instance_double(StravoXExceptionTracker, capture_exception: true) }
      ...
      before do
        ...
        allow(StravoXExceptionTracker).to receive(:new).and_return(exception_tracker)
      end

      it 'captures the exception' do
        expect(StravoXExceptionTracker).to receive(:new).with(error, account: message.account)
