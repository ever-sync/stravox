...
describe StravoXHub do
  ...
  context 'when fetching sync_with_hub' do
    it 'get latest version from stravox hub' do
      version = '1.1.1'
    ...
    it 'returns nil when stravox hub is down' do
  ...
  context 'when fetching captain settings' do
    it 'returns the captain settings' do
      ...
      stub_request(:post, StravoXHub::CAPTAIN_ACCOUNTS_URL).with(
        body: { installation_identifier: described_class.installation_identifier, stravox_account_id: account.id, account_name: account.name }
      ).to_return(
