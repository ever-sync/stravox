...
export const initializeStravoXEvents = () => {
  emitter.on(STRAVOX_RESET, () => {
    if (window.$stravox) {
      window.$stravox.reset();
    }
  });
  emitter.on(STRAVOX_SET_USER, ({ user }) => {
    if (window.$stravox) {
      window.$stravox.setUser(user.email, {
        avatar_url: user.avatar_url,
        email: user.email,
        identifier_hash: user.hmac_identifier,
        name: user.name,
      });
      window.$stravox.setCustomAttributes({
