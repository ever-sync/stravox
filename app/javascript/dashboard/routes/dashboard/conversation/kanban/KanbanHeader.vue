  ...
  <header class="flex flex-col gap-3 mb-4">
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-3 min-h-9">
        <h1 class="text-xl font-semibold leading-9 text-n-slate-12">
        ...
        <div class="relative">
          <select
            ...
            class="appearance-none h-9 bg-n-violet-3/20 border border-n-violet-7/30 rounded-lg pl-3 pr-8 text-xs font-semibold leading-none text-n-violet-11 cursor-pointer hover:bg-n-violet-3/40 focus:outline-none focus:ring-2 focus:ring-n-violet-7/30 transition-colors"
        ...
        <!-- Config button -->
        <button
          class="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg bg-n-violet-3/20 border border-n-violet-7/30 text-xs font-semibold leading-none text-n-violet-11 hover:bg-n-violet-3/40 whitespace-nowrap transition-colors"
          :title="t('CONVERSATION.PIPELINE.CONFIGURE')"
          @click="onOpenConfig"
        >
          <fluent-icon icon="settings" size="16" />
          <span>{{ t('CONVERSATION.PIPELINE.CONFIGURE') }}</span>
        </button>

        <!-- Total count -->
        <span
          v-if="totalCount > 0"
          class="inline-flex items-center justify-center min-w-9 h-9 text-sm font-medium leading-none text-n-slate-10 bg-n-alpha-2 rounded-lg px-3"
        >
          {{ totalCount }}
        </span>
        ...
        <button
          ...
          class="inline-flex items-center gap-1 h-9 px-2.5 text-xs font-medium leading-none text-n-ruby-11 bg-n-ruby-3/20 border border-n-ruby-7/30 rounded-lg hover:bg-n-ruby-3/40 transition-colors"
      ...
      <!-- Controls -->
      <div class="flex items-center gap-3 flex-wrap min-h-9">
        <!-- Search -->
        <div class="relative">
          <fluent-icon
            icon="search"
          ...
          <input
            ...
            class="h-9 bg-n-alpha-1 border border-n-weak rounded-lg pl-8 pr-3 text-xs font-medium leading-none text-n-slate-12 placeholder:text-n-slate-9 w-52 focus:outline-none focus:ring-2 focus:ring-n-violet-7/30 focus:border-n-violet-7 transition-all"
            @input="onSearchInput"
          />
        </div>
        ...
        <div class="relative">
          <select
            ...
            class="appearance-none h-9 rounded-lg pl-3 pr-8 text-xs font-medium leading-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-n-violet-7/30 transition-colors"
            :class="
              selectedInboxId
        ...
        <div class="relative">
          ...
          <fluent-icon
            ...
            class="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none"
            :class="
              selectedTemperature ? 'text-n-violet-10' : 'text-n-slate-10'
            "
          />
        </div>
        ...
        <!-- Assignee tabs -->
        <div
          class="flex items-center h-9 bg-n-alpha-1 rounded-lg border border-n-weak p-0.5 leading-none md:-mt-[17px]"
        >
          <button
            v-for="tab in assigneeTabs"
            :key="tab.key"
            class="h-8 px-3 text-xs font-medium leading-none rounded-md transition-all duration-150"
            :class="
        ...
        <div class="relative">
          <select
            :value="activeSort"
            class="appearance-none h-9 bg-n-alpha-1 border border-n-weak rounded-lg pl-3 pr-8 text-xs font-medium leading-none text-n-slate-11 cursor-pointer hover:border-n-slate-7 focus:outline-none focus:ring-2 focus:ring-n-violet-7/30"
            @change="onChangeSort"
          >
