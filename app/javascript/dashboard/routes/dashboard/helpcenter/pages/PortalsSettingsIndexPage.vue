...
const fetchSSLStatus = () => {
  if (!isOnStravoXCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on StravoX cloud)
  if (portalObj?.custom_domain && isOnStravoXCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
