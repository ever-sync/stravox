...
const latestStravoXVersion = computed(() => {
  return currentAccount.value.latest_stravox_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestStravoXVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestStravoXVersion.value);
});
...
<template>
  <div class="p-4 text-sm text-center">
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_STRAVOX', {
          latestStravoXVersion: latestStravoXVersion,
        })
      }}
    </div>
