...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if ::StravoXApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in StravoX Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if ::StravoXApp.stravox_cloud?
  ...
  enterprise_show_page_attributes = if ::StravoXApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if ::StravoXApp.stravox_cloud?
                                      attrs << :all_features
                                      attrs
                                    else
                                      []
  ...
  enterprise_form_attributes = if ::StravoXApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if ::StravoXApp.stravox_cloud?
                                 attrs << :all_features
                                 attrs
  ...
  def permitted_attributes(action)
    ...
    # Add manually_managed_features to permitted attributes only for StravoX Cloud
    attrs << { manually_managed_features: [] } if ::StravoXApp.stravox_cloud?
